...
<section class="property-wrapper mt-4 pms-wrapper">
    ...
    <h2 class="list-heading">Dealer List</h2>
</section>
<section class="property-wrapper mt-4 user-wrapper">
    <form action="" method="get">
        <div class="d-flex justify-content-between align-items-center container-fluid mt-4 pro-add-nnb">
          <div class="d-flex position-relative">
            <input type="text" name="search" placeholder="Search dealers" class="prop-inp radius-0" value="{{$sort_search}}">
          ...
          <a href="{{route('dealers.create')}}"><button class="btn-add add-btn" type="button"><i class="fa fa-plus" aria-hidden="true"></i>Add Dealer</button></a>
        </div>
    </form>
      ...
      @if($dealers->count() > 0)  
        <table class="mt-4 pro-table-pg table-hover wrapper">
            ...
            @foreach ($dealers as $dealer)             
                <tr>
                    <td scope="row" data-label="ID">{{$dealer->id}}</td>
                    <td data-label="Name">{{$dealer->name ?? "N/A"}}</td>
                    <td data-label="Comment">{{$dealer->comment ?? "N/A"}}</td>
                    <td data-label="Action" class="action-list">
                        <div class="d-inline-flex">
                            <a href="{{route('dealers.edit',$dealer->id)}}" class="icon edit edit-icon">
                                <div class="tooltip">Edit</div>
                                <i class="fa fa-pencil pnsl"></i>
                                </a>
                                <a class="icon edit" onclick="deleteinventory('{{route('dealers.destory',$dealer->id)}}')">
    ...
    @if($dealers->count() > 0 && $dealers->count() > 10)
        <div class="pagination-btn text-center mt-5">
            <a class="btn-add add-btn" href="{{ $dealers->previousPageUrl() }}">Previous</a>
            <a class="btn-add add-btn" href="{{ $dealers->nextPageUrl() }}">Next</a>
        </div>
    @endif
